---
name: gk:visualize
description: Visualize a graph.yaml. Three modes: ASCII (in-session), SVG (visual export, opens in browser), Excalidraw (editable). Use when the user wants to see the graph structure visually — topology, nodes, model tiers, depend_on edges, fan-out/fan-in. Trigger: "visualize graph", "show diagram", "draw the graph", "what does the graph look like".
