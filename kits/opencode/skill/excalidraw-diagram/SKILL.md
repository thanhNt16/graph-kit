...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. This is a hard constraint — LLM output limits per response are finite, and a comprehensive diagram easily exceeds what fits in one shot. Even if it didn't, generating everything at once leads to worse quality. Section-by-section is better in every way.
...
```bash
cd .opencode/skill/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd .opencode/skill/excalidraw-diagram/references
uv sync
