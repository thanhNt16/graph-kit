...
The CLI runs 9 checks: YAML syntax, schema conformance (zod), topology name, agent binding (files in `.omp/agents/`), refs exist on disk, acyclic depend_on, evidence key coverage, loop exit conditions, constraint types.
...
- `agent-binding`: "Agent 'X' not found. Change to a name matching a file in .omp/agents/ (e.g. software-architect)."
