...
writeFileSync(GRAPH, `topology: diamond
...
describe("gk graph waves hook emission", () => {
  ...
  test("waves payload carries advisor and fan_out verbatim", () => {
    ...
    writeFileSync(graph, ["apiVersion: graphkit.dev/v2", "kind: Graph", "metadata:", "  name: orch", "topology: diamond", "nodes:", "  plan:", "    agent: software-architect", "    objective: write briefs", "    model: fable", "  exec:", "    agent: code-reviewer", "    objective: execute briefs", "    depend_on: [plan]", "    model: sonnet", "    loop: { enabled: true, max_rounds: 6, stop_when: \"done\" }", "    advisor: { model: opus, after_failed_rounds: 2, max_calls: 2 }", "    fan_out: { briefs_from: plan, template: \"Implement {brief.title}: {brief.body}\" }"].join("\n"));
    const { stdout, code } = runCli(["graph", "waves", graph, "--json"]);
    expect(code).toBe(0);
    const payload = JSON.parse(stdout).data;
    const nodes = payload.waves.flatMap((w: { nodes: unknown[] }) => w.nodes) as Array<{ id: string; advisor: unknown; fan_out: unknown }>;
