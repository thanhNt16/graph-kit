...
function runCli(args: string[], _cwd?: string) {
  ...
  finally { console.log = origLog; console.warn = origWarn; process.exit = origExit; }
  return { stdout: logs.join("\n"), code: exitCode, error };
}

describe("gk graph commands", () => {
  let tmp: string;
  beforeEach(() => { tmp = join(tmpdir(), `gk-graph-cmd-test-${process.pid}-${Date.now()}`); mkdirSync(tmp, { recursive: true }); scaffoldProject(tmp); });
  afterEach(() => { process.exitCode = 0; rmSync(tmp, { recursive: true, force: true }); });

  test("graph list outputs all 11 topologies", () => {
    const { stdout, code } = runCli(["graph", "list"]);
    expect(code).toBe(0); const parsed = JSON.parse(stdout);
    expect(parsed.status).toBe("ok"); expect(parsed.data.topologies).toHaveLength(11);
    expect(parsed.data.topologies).toContain("diamond"); expect(parsed.data.topologies).toContain("tournament");
  });
  test("graph inspect diamond outputs config keys", () => {
    const { stdout, code } = runCli(["graph", "inspect", "diamond"]);
    expect(code).toBe(0); const parsed = JSON.parse(stdout);
    expect(parsed.status).toBe("ok"); expect(parsed.data.topology).toBe("diamond");
    expect(parsed.data.config_keys).toContain("fanout.strategy"); expect(parsed.data.config_keys).toContain("reduce");
  });
  test("graph inspect nonexistent fails with UNKNOWN_TOPOLOGY", () => {
    ...
    expect(parsed.status).toBe("fail"); expect(parsed.error.code).toBe("UNKNOWN_TOPOLOGY");
  ...
  test("waves rejects an empty graph through schema validation", () => {
    const file = join(tmp, "empty.yaml"); writeFileSync(file, "");
    const out = runCli(["graph", "waves", file]);
    expect(out.code).toBe(1); expect(JSON.parse(out.stdout).error.code).toBe("SCHEMA_INVALID");
